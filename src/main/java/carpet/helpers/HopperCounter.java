...
public class HopperCounter {
    ...
    public List<Text> format(MinecraftServer server, boolean realTime, boolean brief) {
        if (counter.isEmpty()) {
            if (brief) {
                return Collections.singletonList(Messenger.c("g " + name + ": -, -/h, - min "));
            }
            return Collections.singletonList(Messenger.s(String.format("No items for %s yet", name)));
        }
        ...
        List<Text> list = counter.entrySet().stream().map(e -> {
            ...
            return Messenger.s(String.format(" - %s: %d, %.1f/h",
                    itemName,
                    count,
